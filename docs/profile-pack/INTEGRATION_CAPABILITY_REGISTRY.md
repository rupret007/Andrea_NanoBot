...
| Messages / thread context | `degraded_but_usable` | owner-only reply help, bounded summaries, open communication loops, separately approved send/defer decisions | Keep it bounded and point back to Telegram when the bridge is unhealthy | One bounded summary plus one separately approved draft/defer chain |
