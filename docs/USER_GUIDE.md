...
- Say `review recent answers` in the registered main Telegram chat or configured Messages self-thread to review one attributable answer at a time. Opening the review records nothing; only Helpful/Not helpful, a supported tapback, or an explicit fresh verdict becomes learning evidence. A positive verdict can show the next eligible answer; a negative verdict stays focused on its correction path.
