...
description: Non-executable legacy reference for Telegram swarm bots that is blocked pending a reviewed trust-boundary port.
---
...
widens a send-capable MCP tool, copies a pool of bot credentials into legacy
environment storage, and lets subagents perform external sends without a fresh,
target-bound approval. Do not run any credential, source-edit, migration, build,
restart, send, verification, troubleshooting, or removal command below, and do
not collect Telegram bot-pool tokens through this skill.

A reviewed port must keep bot credentials on the host, expose only an exact
route-specific capability, bind every sender identity, chat, and message to a
fresh approval, deny direct sends from ordinary or research routes, and verify
delivery postconditions without silent fallback. It must include credential,
cross-route, approval, replay, and external-send tests before activation.
