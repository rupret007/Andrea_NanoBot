---
name: x-integration
description: Non-executable legacy reference for an X integration that is blocked pending a reviewed trust-boundary port.
---
...
copies skill code into `/app/src`, widens the container MCP surface, exposes a
host browser profile to automation, and performs externally visible mutations.
Do not run any setup, authentication, source-edit, build, restart, post, like,
reply, retweet, quote, test, troubleshooting, or removal command below. Some
historical test commands perform real X actions.

A reviewed port must use the canonical read-only runner and trusted control view,
add exact tools through the route-specific `AssistantRequestPolicy`, keep browser
credentials outside the container, bind every external action to a fresh
target-specific approval, and verify postconditions without silently retrying or
sending. It must include route isolation, credential, approval, and external-send
