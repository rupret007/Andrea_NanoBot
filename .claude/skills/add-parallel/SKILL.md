...
passes an additional provider credential into every runner, creates ambient
remote MCP servers, treats `allowedTools` as a capability boundary, and schedules
an external send without the current route and approval enforcement. Do not run
any credential, source-edit, build, restart, scheduled-task, test, or uninstall
command below, and do not collect a Parallel API key through this skill.

A reviewed port must inject only a selected credential into the specific child
process, configure the SDK's actual `tools` option and an exact MCP allowlist
through `AssistantRequestPolicy`, keep `direct_assistant` tool-free, enforce a
fresh approval and cost cap for paid research and external delivery, and prove
secret redaction and route isolation. It must not rely on writable project
settings, hooks, or legacy runner caches.
