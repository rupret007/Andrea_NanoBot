...
### Adding a Route-Scoped Tool or MCP Integration

Questions to ask:

- What service? (Calendar, Notion, database, etc.)
- What operations needed? (read, write, both)
- Which request route and groups should have access?
- Which operations require fresh approval, and how are credentials isolated?

Implementation:

1. Add the exact built-in and MCP tools to the appropriate
   `AssistantRequestPolicy` classification and maximum in
   `src/assistant-routing.ts`. Ordinary `direct_assistant` turns must remain
   tool-free.
2. Enforce the policy through the SDK's actual `tools` option and exact MCP
   server/tool allowlist in the canonical agent runner. `allowedTools` controls
   auto-approval only; it is not a capability boundary.
3. Use an approved host-side broker or inject only the selected credential into
   the specific child process. Never mount a host credential profile or expose a
   secret through arguments, logs, errors, or diagnostics.
4. Keep runner source, settings, group guidance, skills, and plugins in the
   trusted read-only control view. Do not use project settings or hooks, edit
   `/app/src`, or copy files into preserved session caches.
5. Add tests for exact route tool availability, cross-route isolation, approval
   enforcement, credential redaction, and unavailable-provider behavior.
6. Document the capability and its approval boundary only after those tests
   pass.
