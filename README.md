...
- after a named owed reply, `remind me in 30 minutes`, `remind me in an hour`, or `remind me in half an hour` creates a local owner-chat reminder with a date/time/timezone confirmation. Delays support 1–1,440 minutes or 1–24 hours and accept the `a`/`an`, `half an hour`, and `min`/`mins`/`hr`/`hrs` shorthands, anchored to the original live arrival receipt so retries cannot slide the due time. Missing or expired timing evidence asks for a fresh choice; history-only messages cannot create this reminder. See [short reply delays](docs/COMMUNICATION_COMPANION.md#short-reply-delays).
