...
import { resolveContainerRuntimeName } from '../src/container-runtime.js';
import {
  getPlatform,
  getNodePath,
  getServiceManager,
...
const SERVICE_ID = 'andrea-openai-bot';
const LAUNCHD_LABEL = 'com.andrea-openai-bot';
const STDOUT_LOG = `${SERVICE_ID}.log`;
const STDERR_LOG = `${SERVICE_ID}.error.log`;
const NOHUP_WRAPPER = `start-${SERVICE_ID}.sh`;
const PID_FILE = `${SERVICE_ID}.pid`;
...
<dict>
    ...
    <string>${projectRoot}/logs/${STDOUT_LOG}</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/${STDERR_LOG}</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes(LAUNCHD_LABEL);
  } catch {
...
/**
 * Kill any orphaned Andrea runtime node processes left from previous runs.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned Andrea runtime processes');
  } catch {
 ...
 *
 * Only relevant when Docker is the selected runtime on Linux with user-level
 * systemd. Podman-first setups do not need this check.
 */
function checkDockerGroupSessionStale(): boolean {
  try {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = `/etc/systemd/system/${SERVICE_ID}.service`;
    ...
    unitPath = path.join(unitDir, `${SERVICE_ID}.service`);
    systemctlPrefix = 'systemctl --user';
  }

  const selectedRuntime = resolveContainerRuntimeName();

  const unit = `[Unit]
Description=Andrea OpenAI Bot Runtime
...
StandardOutput=append:${projectRoot}/logs/${STDOUT_LOG}
StandardError=append:${projectRoot}/logs/${STDERR_LOG}
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned runtime processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable ${SERVICE_ID}`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start ${SERVICE_ID}`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active ${SERVICE_ID}`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
  ...
  emitStatus('SETUP_SERVICE', {
    ...
    PROJECT_PATH: projectRoot,
    UNIT_PATH: unitPath,
    CONTAINER_RUNTIME: selectedRuntime,
    SERVICE_LOADED: serviceLoaded,
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, NOHUP_WRAPPER);
  const pidFile = path.join(projectRoot, PID_FILE);

  const lines = [
    '#!/bin/bash',
    `# ${NOHUP_WRAPPER} — Start Andrea OpenAI Bot without systemd`,
    ...
    '    echo "Stopping existing Andrea OpenAI Bot (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting Andrea OpenAI Bot..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/' + STDOUT_LOG)} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/' + STDERR_LOG)} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "Andrea OpenAI Bot started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/${STDOUT_LOG}"`,
  ];
