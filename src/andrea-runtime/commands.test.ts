...
describe('runtime commands', () => {
  ...
  beforeEach(() => {
    ...
    deps = {
      ...
      getExecutionDisabledMessage() {
        return 'Andrea runtime execution is not enabled on this host yet.';
      },
      getRuntimeJobs() {
        return [];
      },
  ...
  it('blocks follow-up when runtime execution is disabled', async () => {
    deps.canExecute = false;

    const handled = await dispatchRuntimeCommand(
      deps,
      'tg:operator',
      '/runtime-followup main continue',
      '/runtime-followup',
    );

    expect(handled).toBe(true);
    expect(sentMessages).toEqual([
      'Andrea runtime execution is not enabled on this host yet.',
    ]);
  });
