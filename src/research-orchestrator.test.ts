...
describe('research orchestrator', () => {
  ...
  it('asks for the options when a generic better-for-me recommendation prompt names no options', async () => {
    const result = await runResearchOrchestrator({
      query: "which one's actually better for me",
      channel: 'telegram',
      groupFolder: 'main',
      now: new Date('2026-04-16T13:07:00.000Z'),
    });
