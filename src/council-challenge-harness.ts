...
const CHALLENGE_SCENARIOS: CouncilChallengeScenario[] = [
  ...
  {
    scenarioId: 'small.gbrain_evidence_contract',
    tier: 'small',
    taskFamily: 'research',
    prompt:
      'Verify the council evidence contract: prefer bounded local evidence, label source priority and citation coverage, and request public search only for a material current gap.',
    expectedCouncilMode: 'single_model',
    requiredRoles: ['openai_cloud'],
    requiredEvidence: 'weak',
    forbiddenLeakageTerms: ['sk-', 'token=', 'password=', 'raw private body'],
    successRubric: [
      'local evidence is considered first',
      'citation and source priority metadata are visible',
      'public search is limited to a material evidence gap',
    ],
    providerBudget: 'low',
    sideEffectPolicy: 'read_only',
    repairPolicy: 'one_approval',
    sourcePatternIds: ['gbrain.brain_first_evidence_contract'],
    sourceRepoIds: ['gbrain'],
  },
  {
    scenarioId: 'small.gbrain_metric_quality_gate',
    tier: 'small',
    taskFamily: 'operator',
    prompt:
      'Verify council quality reporting: metric names need plain-English meaning, family-level gates, actual values, thresholds, and one repair action when a gate misses.',
    expectedCouncilMode: 'single_model',
    requiredRoles: ['openai_cloud'],
    requiredEvidence: 'weak',
    forbiddenLeakageTerms: ['sk-', 'token=', 'password=', 'raw private body'],
    successRubric: [
      ...
      'quality gates expose actual values and thresholds',
      'a failed gate names one bounded repair action',
    ],
    providerBudget: 'low',
    sideEffectPolicy: 'read_only',
    repairPolicy: 'one_approval',
    sourcePatternIds: ['gbrain.metric_glossary_quality_gates'],
    sourceRepoIds: ['gbrain'],
  },
  {
  ...
  {
    scenarioId: 'medium.gbrain_source_conflict_policy',
    tier: 'medium',
    taskFamily: 'research',
    prompt:
      'Review a bounded evidence conflict: preserve both cited claims, compare freshness and authority, lower confidence, and ask for verification instead of silently choosing a winner.',
    expectedCouncilMode: 'dual_review',
    requiredRoles: ['openai_cloud', 'minimax_cloud', 'gemini_cloud'],
    requiredEvidence: 'partial',
    forbiddenLeakageTerms: ['sk-', 'token=', 'password=', 'raw private body'],
    successRubric: [
      'conflicting claims retain evidence identifiers',
      'freshness and authority affect confidence',
      'unresolved conflict becomes an explicit evidence gap',
    ],
    providerBudget: 'medium',
    sideEffectPolicy: 'read_only',
    repairPolicy: 'one_approval',
    sourcePatternIds: ['gbrain.source_attribution_conflict_policy'],
    sourceRepoIds: ['gbrain'],
  },
  {
