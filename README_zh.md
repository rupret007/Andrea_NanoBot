...
- Alexa 主机验证应使用受支持的 Node `22.x`（`>=22 <23`）；仓库、CI 和容器的可复现验证固定使用 `22.22.2`

## Alexa 当前状态

- 仓库侧验证已经很强
- live 证明会过期；请用 `npm run services:status` 查看当前状态。若要宣称完全 live，需要来自真实 Alexa App / 设备 / 已认证模拟器的新签名语音请求
