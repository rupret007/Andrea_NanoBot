...
The historical `/claw` alternate runner is a retired compatibility stub, not an
example to copy or reactivate.
...
Skills that run inside the agent container, not on the host. These teach the
container agent how to use tools, format output, or perform tasks. At launch,
the host builds an atomic trusted control view from the canonical bundled set
plus explicitly enabled community entries, then mounts the route-appropriate
skill and plugin view read-only. It does not create a persistent writable copy
inside each group's session home.
