...
function normalizeChatJid(value: string): string {
  ...
  if (!trimmed) return 'tg:main';
  return trimmed.startsWith('tg:') ? trimmed : `tg:${trimmed}`;
}

function parseArgs(argv: string[]): Args {
  let chatJid = normalizeChatJid(
    process.env.ANDREA_DEBUG_TELEGRAM_CHAT_JID ||
      process.env.TELEGRAM_TEST_CHAT_ID ||
      process.env.TELEGRAM_CANARY_CHAT_ID ||
      'tg:main',
  );
  let limit = 12;
  let json = false;
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--json') {
      ...
      chatJid = normalizeChatJid(argv[index + 1] || chatJid);
      index += 1;
    } else if (arg === '--limit') {
