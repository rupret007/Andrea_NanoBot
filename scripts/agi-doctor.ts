...
const DOCTOR_ENV_KEYS = [
  ...
  'OLLAMA_BASE_URL',
  'TELEGRAM_BOT_TOKEN',
  ...Object.values(INTEGRATION_REQUIREMENTS).flat(),
];

export async function runAgiDoctor(
  env: Record<string, string | undefined> = process.env,
): Promise<DoctorReport> {
  const envFile = env === process.env ? readEnvFile(DOCTOR_ENV_KEYS) : {};
  const effectiveEnv = { ...envFile, ...env };
  const stateDir = expandHome(effectiveEnv.ANDREA_STATE_DIR ?? '~/.andrea');
  const checks: DoctorCheck[] = [];
  checks.push(await checkStateDir(stateDir));
  checks.push(checkModelProviders(effectiveEnv));
  checks.push(checkModelSelection(effectiveEnv));
  checks.push(checkTelegramCanary(effectiveEnv));
  checks.push(...checkIntegrations(effectiveEnv));
...
async function checkAuditChain(auditPath: string): Promise<DoctorCheck> {
  ...
  try {
    ...
    if (s.size === 0)
      return { name: 'audit_chain', status: 'ok', detail: 'empty audit log' };
