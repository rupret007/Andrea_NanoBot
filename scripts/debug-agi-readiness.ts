...
// The gauntlet seeds synthetic reliability/verification records, so by
// default it runs against an isolated in-memory database and never touches
// live ledgers. --live-db opts into reading the real workspace (results are
// still synthetic and clearly labeled).
if (live) {
  initDatabase();
} else {
  _initTestDatabase();
