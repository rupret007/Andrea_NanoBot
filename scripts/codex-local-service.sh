...
LABEL="com.nanoclaw.codex-local"
...
STATE_DIR="${ANDREA_CODEX_SERVICE_STATE_DIR:-$HOME/.andrea/codex-local}"
...
usage() {
  cat <<USAGE
Usage: scripts/codex-local-service.sh <command>

Commands:
  dry-run     Validate config, build, runner, and rendered plist without writes
  install     Install and bootstrap the dedicated loopback LaunchAgent
  status      Show LaunchAgent, health, private-state, and log status
  restart     Restart only the Codex loopback LaunchAgent
  stop        Disable and stop only the Codex loopback LaunchAgent
  uninstall   Stop and remove its plist; preserve worktrees, evidence, and logs

This manager never changes .env and never starts, stops, or restarts NanoClaw.
USAGE
}
...
render_plist() {
  sed \
    ...
    -e "s|{{STATE_DIR}}|$(render_value "$STATE_DIR")|g" \
...
validate_rendered_plist() {
  ...
  if render_plist | grep -q '{{[A-Z_]'; then
    echo "rendered Codex plist contains an unresolved placeholder" >&2
    return 1
  fi
}
...
preflight() {
  ...
  [[ -x "$PROJECT_ROOT/scripts/codex-local-runner.sh" ]] || { echo "Codex runner is not executable" >&2; return 1; }
...
wait_for_ready() {
  ...
  echo "Codex loopback health did not become ready within ${READY_TIMEOUT_SECONDS}s" >&2
  return 1
}
...
stop_service() {
  ...
  if is_bootstrapped; then launchctl bootout "$(service_target)" >/dev/null; fi
  echo "stopped $LABEL"
}
...
status_service() {
  ...
  if [[ -f "$ENTRYPOINT" ]]; then
    ...
    expected_pid="$(launchd_pid || true)"
    if [[ -n "$expected_pid" ]]; then "$pinned_node" "$ENTRYPOINT" --check-health "--expected-pid=$expected_pid" || true; fi
  fi
  [[ -d "$STATE_DIR" ]] && stat -f 'state_permissions=%Lp path=%N' "$STATE_DIR" 2>/dev/null || true
...
uninstall_service() {
  if is_bootstrapped; then launchctl bootout "$(service_target)" >/dev/null 2>&1 || true; fi
  rm -f "$PLIST"
  echo "removed $PLIST; isolated worktrees, evidence, and logs were preserved"
}
