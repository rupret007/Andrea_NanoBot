...
describe("assertSafeUrl", () => {
  ...
  it("blocks literal 127.0.0.1", async () => {
    await expect(assertSafeUrl("https://127.0.0.1/x")).rejects.toThrow(/blocked/);
  });
  it("blocks literal 169.254.169.254 (cloud metadata)", async () => {
    await expect(assertSafeUrl("https://169.254.169.254/latest/")).rejects.toThrow(/blocked/);
  });
  it("blocks literal 10.0.0.5", async () => {
    await expect(assertSafeUrl("https://10.0.0.5/x")).rejects.toThrow(/blocked/);
  });
...
describe("safeFetchText", () => {
  it("times out when fetch never resolves", async () => {
    const neverResolves: typeof fetch = (() =>
      new Promise<Response>(() => { /* never resolves */ })) as typeof fetch;
    ...
    void neverResolves;
    void start;
    await expect(safeFetchText("https://example.com", { fetchImpl: wrapper })).rejects.toThrow();
  });
  ...
  it("truncates response bodies past FETCH_MAX_BYTES", async () => {
    ...
    const r = await safeFetchText("https://example.com", { fetchImpl: stubFetch });
  ...
  it("blocks redirects to local or metadata addresses", async () => {
    ...
    await expect(safeFetchText("https://example.com", { fetchImpl: stubFetch })).rejects.toThrow(/blocked|http/);
  });
});
...
describe("WebResearch integration shape", () => {
  it("declares fetch tool with `external` effect", async () => {
    const ctx = {
      userId: "u",
      scope: "web",
      secrets: { async get(k: string) { return k === "EXA_API_KEY" ? "fake" : undefined; } },
      workdir: "/tmp",
      audit: () => undefined,
    };
